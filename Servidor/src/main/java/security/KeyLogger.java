
package security;

import java.security.PrivateKey;
import java.security.PublicKey;
import javax.crypto.SecretKey;


/**
 * Classe com as chaves cadastradas
 * @author JOAO
 */
public class KeyLogger {
    
    //chave publica e privada do servidor em string
    public static final String ServerPublicKeySTR = "";
    public static final String ServerPrivateKeySTR = "";
    
    //chave publica e privada do servidor
    public static final PublicKey ServerPublicKey = RSAMethods.converteBase64StringParaPublicKey(ServerPublicKeySTR);
    public static final PrivateKey ServerPrivateKey = RSAMethods.converteBase64StringParaPrivateKey(ServerPrivateKeySTR);
    
    public static final String ServerSimetricKeySTR = "T/o3eaaQA9pqprNvXlue8Q==";
    public static final SecretKey ServerSimetricKey = AESMethods.converteBase64ParaSecretKey(ServerSimetricKeySTR);
    
}
